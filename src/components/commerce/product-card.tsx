  ...
  return (
    <Card className="group overflow-hidden">
      <Link href={`/catalog/${product.id}`} className="block">
        <div className="relative aspect-[4/5] overflow-hidden bg-[#F1ECE4]">
          ...
          <div className="absolute left-3 top-3 flex gap-2">
            ...
            {product.stock > 0 ? <Badge variant="success">В наличии</Badge> : <Badge variant="warning">Под заказ</Badge>}
          </div>
        </div>
        ...
        <div className="space-y-4 p-4">
          <div className="space-y-2">
            ...
            <p className="text-sm leading-relaxed text-[#766A5F]">{product.description}</p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 text-xs text-[#766A5F]">
            ...
            <div className="rounded-[2px] bg-[#F1ECE4] px-3 py-2">
              <div className="text-[10px] font-bold uppercase tracking-widest">Размер</div>
              <div className="mt-1 font-semibold text-[#2D251E]">
                {product.dimensions.length} x {product.dimensions.width} x {product.dimensions.height} мм
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="flex items-end justify-between gap-4 border-t border-[rgba(166,106,63,0.18)] px-4 pb-4 pt-4">
        <div>
          <div className="text-xl font-bold text-[#8E623E]">{formatPrice(product.discountPrice || product.price)}</div>
          <div className="text-xs text-[#766A5F]">{product.finish}</div>
        </div>
        <RoleActionButton product={product} compact />
      </div>
    </Card>
  );
}
